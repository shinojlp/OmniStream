  ...
  const handleSearch = (search) => {
    if (search && search.length > 2) {
      ...
      searchMovies({
        ...
        // console.log("resulsts value", results);
      });
    } else {
      setLoading(false);
      setResults([]);
  ...
  return (
    <SafeAreaView className="bg-neutral-800 flex-1">
      ...
      <View className="mx-4 mb-3 flex-row justify-between items-center border border-neutral-500 rounded-full">
        ...
        />
        <TouchableOpacity onPress={() => navigation.navigate("Home")} className="rounded-full p-3 m-2 bg-neutral-500"
            ...
            {results.map((item, index) => {
              return (
                <TouchableWithoutFeedback key={index} onPress={() =>
                  item.title ?
                    navigation.push("Movie", item) : navigation.push("Series", item)}>
                  <View className="space-y-2 mb-4">
                    ...
                    {console.log(item, item.title)}
                    {
                      item.title ? (
                        ...
                        <Text className="text-gray-300 ml-1">
                          {item.name && item.name.length > 22 ? item.name.slice(0, 22) + "..." : item.name}
                        </Text>
                      )
                    }
